...
if [[ "$MODE" == "on" ]]; then
  ...
  cp /usr/local/etc/nftables_forward.conf.disabled /etc/nftables.conf
  ...
  #Start DHCP server
  cp /usr/local/etc/dnsmasq_DHCP.conf.disabled /etc/dnsmasq.d/lan-$LAN.conf
  systemctl restart dnsmasq
  echo "DHCP server enabled"
  #keep networkmanager away from $LAN
  cp /usr/local/etc/netman_unmanaged.conf.disabled /etc/NetworkManager/conf.d/unmanaged.conf
